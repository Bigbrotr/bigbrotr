...
async def get_start_time_async(
    ...
    Returns:
        int: The timestamp to start synchronization from (always a positive integer)
    """
    for attempt in range(retries):
        try:
            ...
            if attempt + 1 < retries:
                ...
                logging.error(
                    f"❌ Final attempt {attempt + 1}/{retries} failed while getting start time for {relay.url}: {e}")
                # Return default_start_time instead of raising exception
                return default_start_time

    # Fallback: ensure we always return a positive integer
    return default_start_time
