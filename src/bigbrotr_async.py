"""Async version of Bigbrotr database wrapper using asyncpg for better performance."""
import asyncpg
import json
import time
...
class BigbrotrAsync:
    ...
    def __init__(
        ...
        """Initialize BigbrotrAsync instance with connection parameters."""
    ...
    async def insert_relay(self, relay: Relay, inserted_at: Optional[int] = None) -> None:
        ...
        await self.execute(query, sanitize(relay.url), relay.network, inserted_at)
    ...
    async def insert_relay_batch(
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                await conn.executemany(
                    query,
                    [(sanitize(relay.url), relay.network, inserted_at) for relay in relays],
                )
