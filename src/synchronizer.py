...
def relay_worker_thread(config: Dict[str, Any], shared_queue: Queue, end_time: int) -> None:
    ...
    try:
        ...
        loop.run_until_complete(connect_bigbrotr_with_retry(bigbrotr, logger=logging))
...
async def main_loop(config: Dict[str, Any]) -> None:
    ...
    # Load priority relays if path is configured
    priority_relays = []
    if config.get("priority_relays_path"):
        priority_relays = await fetch_relays_from_file(config["priority_relays_path"])
    else:
        logging.warning("⚠️ No priority relays path configured")
