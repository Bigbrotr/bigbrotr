...
-- Description: Stored procedures for batch operations (no tags/content)
...
CREATE OR REPLACE FUNCTION insert_event(
    ...
    p_created_at            BIGINT,
    p_kind                  INTEGER,
    p_tags                  JSONB,      -- Accepted but NOT stored (LilBrotr)
    p_content               TEXT,       -- Accepted but NOT stored (LilBrotr)
    p_sig                   BYTEA,
    p_relay_url             TEXT,
...
AS $$
BEGIN
    -- Insert event (idempotent) - tags and content are NOT stored in LilBrotr
...
COMMENT ON FUNCTION insert_event IS 'Atomically inserts event (tags/content NOT stored), relay, and their association';
